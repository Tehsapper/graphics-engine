...
void ContextEventObserver::charCallback(GLFWwindow* win, uint32_t utf_codepoint) {
	callbacks[win]->onChar(utf_codepoint);
}
...
void ContextEventObserver::onChar(uint32_t utf8_codepoint) const {
	for (auto* observer : char_observers) {
		observer->onChar(utf8_codepoint);
	}
}
